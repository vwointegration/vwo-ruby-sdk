...
class VWOTest < Test::Unit::TestCase
  ...
  def set_up(config_variant = 1)
    ...
    @goal_identifier = SETTINGS_FILE[config_variant.to_s]['campaigns'][0]['goals'][0]['identifier']
  rescue
    # Do nothing
  end
  ...
  def test_init_vwo_with_invalid_settings_file
    set_up(0)
    assert_equal(@vwo.is_valid, false)
  end

  # Test get_variation
  def test_get_variation_invalid_params
    set_up()
    assert_equal(@vwo.get_variation(123, 456), nil)
  end

  def test_get_variation_invalid_config
    set_up(0)
    assert_equal(@vwo.get_variation(@user_id, 'some_campaign'), nil)
  end

  def test_get_variation_with_no_campaign_key_found
    set_up(1)
    USER_EXPECTATIONS[@campaign_key].each do |test|
      assert_equal(@vwo.get_variation('NO_SUCH_CAMPAIGN_KEY', test['user']), nil)
    end
  end
  ...
  def test_get_variation_against_campaign_traffic_50_and_split_50_50
    set_up(1)
    USER_EXPECTATIONS[@campaign_key].each do |test|
      assert_equal(@vwo.get_variation(@campaign_key, test['user']), test['variation'])
    end
  end
